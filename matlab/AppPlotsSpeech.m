...
goodputAll  = throughputBrowser+throughputFTP;
...
eR1 = [eR1; (e(2,:) ./ x) .*  (x ./ e(3,:))];
meanR1 = [meanR1; mean(9,:)];
eR1 = [eR1; e(9,:)];

param = 'FTP Throughput';
xlab = '# of clients';
ylab = 'MB/s';
l = {'Combined HTTP Throughput', 'FTP Throughput'};
...
param = 'HTTP Throughput';
xlab = '# of clients';
ylab = 'MB/s';
l = {'Combined HTTP Throughput','Single HTTP Throughput'};
%combPlotConf(imageDirectory, title, x, meanR, eR, '# of clients', yunit, legend);         
figure('Name',param)
hold on;
%bar(x, mean,'facecolor',[.8 .8 .8]); ylabel(ylab); xlabel(xlab);
...
param = 'Video Conference Packet Loss Rates';
xlab = '# of clients';
ylab = '%';
...
hold on;
%bar(x, mean,'facecolor',[.8 .8 .8]); ylabel(ylab); xlabel(xlab);
errorbar(x,meanR2(3,:),eR2(2,:),'Color', '1.0 0.7 0.0' ,'LineWidth',1);
ylabel(ylab);
xlabel(xlab);
legend(l, 'Location','northwest');
title(param,'FontSize', titleSize);
