...
\begin{figure}[!ht]
  ...
  \caption{Throughput of the FTP and web browsing applications} \label{fig:app_throughput}
\end{figure}
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.6\textwidth]{../matlab/images/finalnocctv/Browser -average session duration.png}
  \caption{Throughput of the FTP and web browsing applications} \label{fig:browser_sess_dur}
\end{figure}
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.6\textwidth]{../matlab/images/finalnocctv/Application Packet Delays.png}
  \caption{Throughput of the FTP and web browsing applications} \label{fig:app_pkt_delay}
\end{figure}
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.6\textwidth]{../matlab/images/finalnocctv/Application Packet Loss Rates.png}
  \caption{Throughput of the FTP and web browsing applications} \label{fig:app_pkt_loss}
\end{figure}
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.6\textwidth]{../matlab/images/finalnocctv/RAP -Average Queue Length.png}
  \caption{Throughput of the FTP and web browsing applications} \label{fig:rap_queue_len}
\end{figure}



In order to analyze the overall network behavior, it is useful that we first familiarize ourselves with the behavior of its components, i.e. the applications described in section \ref{subsec:apps}.
...
\paragraph{The FTP client}, shown in fig. \ref{fig:app_throughput} as well, experiences a (TODO: characteristic) decline in throughput that is inverse to the increase in overall HTTP throughput. This can be explained with TCP's congestion control mechanisms: the more bandwidth is used by web browsing clients, the more the FTP client is forced to react by limiting its bandwidth in order to avoid collisions. In short: the FTP client will use up as much bandwidth as you let them.
...
% TODO: kann man ableiten dass die queue aus gründen logarithmusch (und nciht zB quadratisch) wächst?)
%TODO: bis 20 linear, dann beschränkt?
TODO: explain packet loss!!

\subsubsection{Application influence on video quality}


\subsubsection{Maxmimum number of web browsing clients}
bevor video conf gefährdet ist bzw bis studi browsing erlebnis unbrauchbar ist
   3.2 was die bottlenecks sind (wann, wo?), ob PPP eins davon ist

\subsubsection{Identified bottlenecks}

\subsubsection{QoS improvement suggestions}
 wie kann man die vieo QoS verbessern, wie die browsing QoS?
